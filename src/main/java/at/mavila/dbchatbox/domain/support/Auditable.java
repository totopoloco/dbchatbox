...
/**
 * Abstract JPA base class that provides automatic audit timestamps for all
 * mutable domain entities.
 *
 * <p>
 * Subclasses inherit {@code createdAt} and {@code updatedAt} columns, which are
 * populated by JPA lifecycle callbacks —
 * no application code sets them explicitly. {@code createdAt} is written once
 * at insert and never modified;
 * {@code updatedAt} is refreshed on every update.
 * </p>
 *
 * <p>
 * Annotated with {@link MappedSuperclass} so JPA maps the inherited columns
 * into each owning entity's table — no
...
public abstract class Auditable {
   ...
   * {@link #prePersist()} and never modified
   * afterwards — the {@code updatable = false} column constraint prevents
   * accidental overwrites by JPA.
   *
   * <p>
   ...
   * {@link #createdAt} in
   * {@link #prePersist()} and refreshed on every subsequent update by
   * {@link #preUpdate()}.
   *
   * <p>
   ...
   * Refreshes {@link #updatedAt} to the current
