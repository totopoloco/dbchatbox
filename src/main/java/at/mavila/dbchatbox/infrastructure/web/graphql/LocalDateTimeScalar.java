 ...
 *
 * <p>
 * {@code ExtendedScalars.DateTime} only accepts {@code OffsetDateTime}, but every {@code DateTime!} field in the schema
 * is backed by a Java {@code LocalDateTime}. This scalar handles both types — {@code LocalDateTime} is serialized as an
 * ISO-8601 string without an offset (e.g. {@code "2026-05-01T11:05:32.123456"}); {@code OffsetDateTime} is converted to
 * local date-time first to retain backward compatibility.
 * </p>
 *
 * <p>
 * The scalar is named {@code "DateTime"} to match the {@code scalar DateTime} declaration in the schema. No schema
...
public class LocalDateTimeScalar {
  ...
  public GraphQLScalarType toGraphQLScalarType() {
    return GraphQLScalarType.newScalar().name("DateTime")
        ...
        .coercing(new LocalDateTimeCoercing()).build();
  }
  ...
  private static final class LocalDateTimeCoercing implements Coercing<LocalDateTime, String> {
    ...
    public LocalDateTime parseValue(final Object input, final GraphQLContext context, final Locale locale)
      ...
      throw new CoercingParseValueException(
          "Expected a String for DateTime input but got '%s'.".formatted(input.getClass().getName()));
    }
