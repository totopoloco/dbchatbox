...
- `src/main/resources/db/migration/V1..V6_*.sql` — Flyway versioned migrations (`ddl-auto=validate`, so schema changes require a new migration)
- `src/specs/club/ClubManagement.md` — Phase 1 product spec
- `src/specs/ClubManagement-Phase2-Multitenancy-Auth.md` — Phase 2 spec (Keycloak + multi-tenancy, pending implementation)
...
- All mutable entities extend `Auditable` (`@MappedSuperclass` providing `createdAt`/`updatedAt` via `@PrePersist`/`@PreUpdate`)
